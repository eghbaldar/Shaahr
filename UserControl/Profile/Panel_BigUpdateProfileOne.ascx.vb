...
Partial Class UserControl_Profile_Panel_BigUpdateProfileOne
    ...
    Protected Sub Dl_UpdateNews_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataListItemEventArgs) Handles Dl_UpdateNews.ItemDataBound

        Dim linkDelete As HtmlAnchor = DirectCast(e.Item.FindControl("linkDelete"), HtmlAnchor)
        Dim btnDelete As ImageButton = DirectCast(e.Item.FindControl("btnDeleteObject"), ImageButton)
        Dim prompt As String = "$.prompt('آیا از حذف کامل این پست مطمئن هستید؟ " + _
                       "<input type=""hidden"" value=""{0}"" name=""hidID"" />'" + _
                         ", {{buttons: {{ آری: true, خیر: false }}, callback: confirmDeleteResult}} ); return false; "
        linkDelete.Attributes("onclick") = String.Format(prompt, btnDelete.ClientID)
        '-------
        Dim txt As Panel = CType(e.Item.FindControl("PnlComment"), Panel)
        Dim strChange As String = String.Format("document.getElementById('ctl00_ContentPlaceHolder1_Panel_BigUpdateProfileOne1_Dl_UpdateNews_ctl{0}_PnlbtnComment').style.display='inline';", Format(e.Item.ItemIndex, "00"))
        txt.Attributes.Add("onmousemove", strChange)
    ...
    Public Sub ShowAllComment(ByVal Sender As Object, ByVal e As CommandEventArgs)
        ...
        For i As Byte = 0 To Dl_UpdateNews.Items.Count - 1
            Dim lbl As Label = CType(Dl_UpdateNews.Items(i).FindControl("lbl_status_Comment"), Label)
